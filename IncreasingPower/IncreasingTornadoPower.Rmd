The Increasing Power of U.S. Tornadoes
======================================

https://medium.com/@arinbasu/a-tutorial-on-how-to-interface-an-r-notebook-with-overleaf-11f23c306cfd

See also: IncreasingEnergy.Rmd, AnnualQuantilesEnergyDissipation.Rmd
https://simplystatistics.org/2018/04/17/what-is-a-successful-data-analysis/
https://www.youtube.com/watch?v=1p_Us5WFQ7w&list=PL3IqNVB6Zfz-nRkuz4F1Vc5ar1zCugV_a&index=3

Get packages.
```{r}
setwd("~/Dropbox/Tornadoes/IncreasingPower")
library(tidyverse)
library(lubridate)
library(rgeos)
library(sf)
...
Write out energy dissipation.
```{r, eval=FALSE}
df <- as.data.frame(Tor.sfdf)
write.csv(df, file = "Tornadoes1994-2016.csv")
```
...
#library(ggimage)
#img <- "TornadoShelter.jpg"
#ggbackground(p, img)
#ggbackground(p, img,
#             image_fun = function(x) magick::image_negate(magick::image_convolve(x, 'DoG:0,0,2')))
...
#  scale_y_log10(breaks = c(.1, 10, 1000), labels = c(.1, 10, 1000)) +
...
#  ggtitle("C")
#  ggtitle("Quantiles of Tornado Energy Dissipation By Month",
#          subtitle = "Median, Interquartile Range, 90th Percentile")
print(p2)
#ggplot(df, aes(x = yr, y = nT)) +
#  geom_point() +
#  scale_x_continuous(breaks = seq(1995, 2015, 20)) +
#  xlab("Year") + ylab("Energy Dissipation [GW]") +
#  facet_wrap(~ mo, ncol = 12) +
#  theme_minimal() +
#  ggtitle("Number of Tornadoes By Month")
...
p3 <- ggplot(df, aes(x = Hour, y = q50ED/10^9)) +
  ...
  theme_minimal() +
  ggtitle("B")
#  ggtitle("Quantiles of Tornado Energy Dissipation By Hour",
#          subtitle = "Median, Interquartile Range, 95th Percentile")
print(p3)
```
...
Putting multiple plots together in a patchwork. https://github.com/thomasp85/patchwork/
```{r}
...
SST bbox (SST5): 10, 35, -97, -70  https://www.esrl.noaa.gov/psd/cgi-bin/data/timeseries/timeseries1.pl
```{r}
...
SST <- read.table("SST5.txt", header = TRUE) %>%
  filter(Year >= 1994)
SST.df <- reshape2::melt(SST, id.vars = "Year")
names(SST.df) <- c("yr", "mo.abb", "SST")

PW <- read.table("VMomentum.txt", header = TRUE) %>%
  filter(Year >= 1994)
PW.df <- reshape2::melt(PW, id.vars = "Year")
names(PW.df) <- c("yr", "mo.abb", "PW")

#df$mo.abb <- month.abb[df$mo]

df <- as.data.frame(Tor.sfdf) %>%
  mutate(mo.abb = month.abb[mo]) %>%
  left_join(ENSO.df)
df <- left_join(df, SST.df)
df <- left_join(df, PW.df)
df$After2007 <- df$yr >= 2007
```

Change point in 2007. Use of the Enhanced Fujita Scale.

Northern Hemisphere Sea Ice Extent https://www.esrl.noaa.gov/psd/data/climateindices/
```{r}
Ma <- factor(month.abb, levels = month.abb[1:12])
NHSIE <- read.table("NHSIE.txt", header = TRUE) %>%
  mutate(Mo = rep(Ma, 38),
         Year = trunc(Year)) %>%
  spread(., key = Mo, value = Value)
NHSIE.df <- reshape2::melt(NHSIE, id.vars = "Year")
names(NHSIE.df) <- c("yr", "mo.abb", "NHSIE")
df <- left_join(df, NHSIE.df)
```


The geometric standard deviation describes the spread in the set of energy dissipation values as a (dimensionless) multiplicative factor relative to the geometric mean [1]. The one standard deviation range about the mean is obtained by dividing and multipling the geometric mean by the geometric standard deviation. [1] Kirkwood, T.B.L. (1979). "Geometric means and measures of dispersion". Biometrics. 35: 908–9. JSTOR 2530139. 
...
```{r}
library(lme4)
model0 <- lmer(log(ED) ~ ENSO + I(yr - 2002) + (1|After2007) + (1|mo), data = df)
summary(model0)


model0a <- lmer(log(ED) ~  ENSO + (1|After2007) + (I(yr - 2002)|mo) + (1|Hour), data = df)

model0a <- lmer(log(ED) ~ I(yr - 2002) + SST + After2007 + (1|mo) + (1|Hour), data = df2)
model0a <- lmer(log(ED) ~ I(yr - 2002) + SST + ENSO + After2007 + (1|mo) + (1|Hour), data = df)

summary(model0a)
ranef(model0a)

exp(predict(model0a, newdata = data.frame(ENSO = .05371878, yr = 2016, After2007 = TRUE, mo = 2, Hour = 15, SST = c(20.5, 22))))/10^9
```
...
There is no relationship with total number of tornadoes.

The wiggly parts of the spline basis are treated as a random effect and their associated variance parameter `sigma` controls the degree of wiggliness of the fitted spline. The perfectly smooth parts of the basis are treated as a fixed effect (`smo_1`).

https://www.fromthebottomoftheheap.net/2018/04/21/fitting-gams-with-brms/

Save the current workspace.
```{r}
#save.image(file = "May2.RData")
...
ggplot(coefTable, aes(x = id, y = mi)) +
  ...
  coord_flip() 
#  ggtitle("Tornado Energy Dissipation", subtitle = "Fixed Effects")
```
#### Figure 3
...
p2 + geom_ribbon(aes(x = yr, ymin = lower__/10^9/scalingFactor , ymax = upper__/10^9/scalingFactor), 
     ...
     geom_line(aes(x = yr, y = estimate__/10^9/scalingFactor), data = TrendmarginalF, col = "black", inherit.aes = FALSE)

     geom_line(aes(x = yr, y = estimate__/10^9), data = TrendmarginalP, col = "black", inherit.aes = FALSE) 
...
Working with Bayes model output in the tidy format. http://mjskay.github.io/tidybayes/
...
df %>%
  modelr::data_grid(yr = modelr::seq_range(yr, n = 22), ENSO = 0, After2007 = TRUE, mo = 1:12, Hour = 20) %>%
  add_predicted_samples(model) %>%
  ggplot(aes(x = yr)) +
  stat_lineribbon(aes(y = pred/10^9), .prob = c(.99, .95, .8, .5)) +
  geom_point(aes(y = ED/10^9), data = df, alpha = .2, size = .2) + 
  scale_y_log10(limits = c(.0001, NA), 
                breaks = c(.0001, .1, 100, 100000),
                labels = c("0.0001", "0.1", "100", "100,000")) +
  facet_wrap(~ mo, labeller = as_labeller(month.name)) +
  ylab("Energy Dissipation (GW)") + xlab("") +
  scale_fill_brewer() +
  theme_minimal()

names(month.abb) <- 1:12

df %>%
  modelr::data_grid(After2007 = c(FALSE, TRUE), ENSO = 0, yr = 2017, mo = 1:12, Hour = 20) %>%
  add_predicted_samples(model) %>%
  ggplot(aes(x = After2007)) +
  stat_pointinterval(aes(y = pred/10^9), .prob = c(.99, .95, .8, .5), col = "red") +
#  geom_point(aes(y = ED/10^9), data = df, alpha = .2, size = .2) + 
  scale_y_log10(limits = c(.0001, NA), 
                breaks = c(.0001, .1, 100, 100000),
                labels = c("0.0001", "0.1", "100", "100,000")) +
  scale_x_discrete(labels = c("1994-2006", "2007-2016")) +
  facet_wrap(~ mo, labeller = as_labeller(month.abb), ncol = 12) +
  ylab("Conditional Energy Dissipation (GW)") + xlab("") +
  scale_fill_brewer() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
df2 <- df %>%
  filter(mag >= 0)
formula <- ED ~ ENSO + SST + After2007 + I(yr - 2002) +
...
No correspondence to the frequency of tornadoes.
```{r}
df2 <- df %>%
  group_by(Ma) %>%
  summarize(nT = n(),
            After2007 = first(After2007),
            ENSO = first(ENSO),
            yr = first(yr),
            WCA = first(WCA))

formula <- nT ~ ENSO + After2007 + WCA +
                f(Ma, model = "seasonal", season.length = 12, param = c(1, .1)) 
family <- "poisson"

modeli2 <- inla(formula = formula, 
                family = family,
                data = df2)
...
```{r}
plot(modeli$marginals.fixed$`I(yr - 2002)`)
plot(modeli$marginals.random$Hour$index.1)
```
